...
export function Task({ todo, todos, setTodos }: TaskProps) {
  const [openModal, setOpenModal] = useState(false);

  function handleTaskChecked(id: string) {
    const localTodos = [...todos];

    const todoFiltered = localTodos.filter(localTodo => localTodo.id === id);
    todoFiltered[0].isCompleted = !todoFiltered[0].isCompleted;
    
    const todoIndex = localTodos.findIndex(localTodo => localTodo.id === id);
    localTodos.splice(todoIndex, 1 , todoFiltered[0]);
    
    setTodos(localTodos);
    localStorage.setItem('todos', JSON.stringify(localTodos));
    todoFiltered[0].isCompleted ? notify('Tarefa concluída!', 'info') : '';
  }

  function handleDeleteTask(id: string) {
    const localTodos = [...todos];

    const todosUpdated = localTodos.filter(localTodo => localTodo.id !== id);
  ...
  return (
    <>
      {openModal && <Modal setOpenModal={setOpenModal} todo={todo} onDeleteTask={handleDeleteTask} />}      
        ...
        <div className={styles.checkboxContainer}>
          <form>
            <input 
              ...
              onClick={() => handleTaskChecked(todo.id)}
            />
            <span className={styles.checkmark}></span>
          </form>
        </div>
        <p className={todo.isCompleted ? styles.scratchedTextDisabled : ""}>{todo.description}</p>
